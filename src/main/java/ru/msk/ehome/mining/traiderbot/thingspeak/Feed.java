...
public class Feed {
	...
	/**
	 * Although there are more sensible ways to build this class, the structure must
	 * match the JSON returned by the ThingSpeak API to allow GSON to de-serialize
	 * it.
	 */
	...
	/**
	 * Constructor is package/class private so it can only be created via GSON and
	...
	/**
	 * Get channel description. Channel description can be set via the ThingSpeak
	 ...
	 * @param field
	 *            1-8.
	 * @return The assigned name of the field; null for status feeds or if the field
	 *         is unassigned.
	 */
	...
	/**
	 * Get the name of this channel. Set a name for the channel using the ThingSpeak
	 ...
	 * {@link FeedParameters#offset(java.lang.Integer)} to adjust to local timezone.
	 *
	 ...
	 * many different entries, this method could be quite slow. It may be better to
	 * call {@link #getEntryMap()} to obtain a map of entries indexed by id.
	 *
	 * @param id
	 *            Entry ID.
	 * @return Entry.
	 * @throws ThingSpeakException
	 *             if the feed does not contain an Entry with the given id.
	 */
	...
	/**
	 * Get the last / latest entry in this feed. If you only need the last entry and
	 * not the rest of the feed, consider using
	 * {@link Channel#getLastChannelEntry()}.
	 *
	 * @return An Entry with id equal to the feed's last_entry_id.
	 * @throws ThingSpeakException
	 *             The channel does not have a last entry or the feed is a Status
	 *             feed.
	 */
