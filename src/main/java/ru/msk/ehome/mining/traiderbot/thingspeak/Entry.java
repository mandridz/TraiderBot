...
public class Entry {
	...
	/**
	 * The names of these private members must match the JSON fields in a channel
	 * feed returned by ThingSpeak. If they don't, GSON might not be able to
	 * deserialize the JSON feed into Entry objects. Note that 'longitude' and
	 * 'latitude' are returned by feeds, but 'lat' and 'long' are used when
	 * updating.
	 */
	 ...
	 * @param field
	 *            1-8
	 * @return Field data; null for status feeds, undefined fields, and field feeds
	 *         where field was not specified.
	 */
	 ...
	 * Get latitude.
	 * 
	 * @return Latitude, in decimal degrees; 0.0 if undefined; null for status feeds
	 *         or if location info was not requested using
	 ...
	 *         location info was not requested using
	 ...
	 * Get status.
	 * 
	 * @return Status string; null for Channel and Field feeds if status info was
	 *         not requested using {@link FeedParameters#status(java.lang.Boolean)}
	 */
	...
	/**
	 * Set Twitter username. If set, a tweet will be posted to the user's twitter
	...
	/**
	 * Set Twitter message. This message will be posted to the user's twitter feed
	...
	/**
	 * Set the created date of an entry. If not explicitly set, the channel update
	 * time is used. Useful when entries are not created and updated at the same
	 * time (offline mode, queuing to avoid rate-limiting, etc.)
	 * 
	 * @param created
	 *            date which will be send to thingspeak
	 */
